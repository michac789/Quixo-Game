...
def menu():
    ...
    while(True): 
        ...
        # Single player (vs computer level 1/2/3/4)
            ...
            #player1lv = int(input("Enter player 1 level: "))
            #player2lv = int(input("Enter player 2 level: "))
            winnerplayer1 = 0
            winnerplayer2 = 0
            for player1lv in range(1, 5):
                for player2lv in range(1, 5):
                    winnerplayer1 = 0
                    winnerplayer2 = 0
                    for m in range(10):
                        board = [0 for i in range(int(n)*int(n))]
                        ...
                        while(True):
                            turn = turn % 2 + 1
                            #print(f"Computer level 2 is thinking...")
                            compindex, comppush = computer_move(board, turn, player1lv)
                            ...
                            if check_victory(board, turn) != 0:
                                winnerplayer1 += 1
                                #display_board(board)
                                #print(f"Winner is {check_victory(board, turn)}")
                                break

                            turn = turn % 2 + 1
                            #print(f"Computer level 3 is thinking...")
                            compindex, comppush = computer_move(board, turn, player2lv)
                            #print(f"Computer move: row {compindex // int(n) + 1}, column {(compindex % int(n)) + 1}, push_from {comppush}")
                            board = apply_move(board, turn, compindex, comppush)
                            #display_board(board)
                            if check_victory(board, turn) != 0:
                                ...
                                #print(f"Winner is {check_victory(board, turn)}")
                                break
                    print(f"Winner lv {player1lv} = {winnerplayer1}")
                    print(f"Winner lv {player2lv} = {winnerplayer2}")
                    print(f"")
        else:
